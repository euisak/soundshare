...
import { avatarLetter, timeAgo, escHtml, showToast, resolveAvatars, renderFeedCard, renderPagination } from "./ui.js";
...
mypageAvatarEl.textContent = avatarLetter(name);
...
// ── 탭별 ⋮ 드롭다운 아이템 빌더 ──────────────────────────────
function buildDropdownItems(post, tab) {
  const pid     = escHtml(post.id);
  const isOwner = post.authorId === user.uid;
  if (tab === "liked") {
    return `<button class="post-list-dropdown-item danger" data-action="unlike-post" data-id="${pid}">좋아요 취소</button>`;
  }
  if (isOwner) {
    return `<a class="post-list-dropdown-item" href="write.html#${pid}">수정</a>
            <button class="post-list-dropdown-item danger" data-action="delete-post" data-id="${pid}">삭제</button>`;
  }
  return "";
}
...
function renderTab(tab) {
  ...
  content.innerHTML = items + renderPagination(data.length, page, PER_PAGE, tab);
  resolveAvatars(content);
}
